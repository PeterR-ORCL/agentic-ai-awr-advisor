...
class DashboardScreen6ModelRegistryReviewPanelTests(unittest.TestCase):
    def test_html_dashboard_compiles(self) -> None:
        py_compile.compile(str(HTML_DASHBOARD_PATH), doraise=True)

    def test_screen6_model_registry_review_preview_exists(self) -> None:
        source = read_text(HTML_DASHBOARD_PATH)
        rendered = self.render_screen6()
        for phrase in (
            "Screen 6 ML Model Registry Review Preview",
            "Read-Only Model Registry Review Request Preview",
            "Phase 7BN Preview",
            "model_id",
            "model_family",
            "validation_status",
    ...
    def test_controls_are_disabled_preview_only(self) -> None:
        rendered = self.render_screen6()
        self.assertIn('id="screen6-model-registry-review-preview-panel"', rendered)
        self.assertEqual(
            rendered.count("screen6-model-registry-review-control preview-only"),
            9,
        )
        self.assertGreaterEqual(rendered.count("disabled"), 9)
        self.assertGreaterEqual(rendered.count('aria-disabled="true"'), 9)
        self.assertGreaterEqual(rendered.count('data-preview-only="true"'), 10)
        self.assertIn("Controls are disabled/preview-only", rendered)
        self.assertIn("Model registry review disabled in this phase", rendered)
    ...
    def test_no_fetch_xhr_forms_or_backend_calls(self) -> None:
        ...
        panel_source = inspect.getsource(
            dashboard._render_screen6_model_registry_review_preview_panel
        ).lower()
        ...
        for phrase in (
            ...
            "sendbeacon",
            "/api/",
            "model_registry_review_endpoint",
