...
def test_screen5_renderer_renders_recommendation_action_evidence():
...
def test_screen5_renderer_renders_missing_unavailable_and_stale_action_states():
    missing = render_screen5_action(action_model())
    unavailable = render_screen5_action(
        action_model(recommendation_action_payload={"llm_recommendation": "invented", "rows": [{"row_id": "x"}]})
    )
    stale = render_screen5_action(
        action_model(
    ...
    assert "invented" not in unavailable
...
def test_screen5_renderer_does_not_render_llm_text_as_recommendation():
    ...
    assert "Recommendation fields" not in html
