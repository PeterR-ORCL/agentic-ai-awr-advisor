...
The Phase 7H.1 foundation supports these state keys for future use: `selectedAwr`, `selectedRun`, `selectedDb`, `selectedSystem`, `selectedSnapshot`, `selectedDomain`, `selectedSeverity`, `selectedRecommendation`, `selectedGovernanceItem`, `selectedSemanticItem`, `selectedLearningCandidate`, `selectedFleetGroup`, and `selectedComparisonBaseline`.
