"""Screen 5 recommendation/action view model.

This module exposes deterministic recommendation/action evidence from an
evidence_pack_contract. It does not create recommendations, call LLMs, infer
actions from UI state, render HTML, or mutate source contracts.
"""
...
class Screen5ActionViewModel:
    ...
    recommendation_fields: tuple[dict[str, Any], ...]
    risk_fields: tuple[dict[str, Any], ...]
    impact_fields: tuple[dict[str, Any], ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "recommendation_fields": list(self.recommendation_fields),
            "risk_fields": list(self.risk_fields),
...
def build_screen5_action_view_model(evidence_pack: EvidencePackContract) -> Screen5ActionViewModel:
    """Build the Screen 5 action view model from recommendation/action evidence."""

    pack = validate_evidence_pack_contract(evidence_pack)
    domain = EvidenceDomain.RECOMMENDATION_ACTION
    section = pack.recommendation_action_evidence
    rows = tuple(row.to_dict() for row in section.rows)
    return Screen5ActionViewModel(
        ...
        action_rows=rows,
        recommendation_fields=_fields_matching(rows, ("recommendation", "action", "outcome")),
        risk_fields=_fields_matching(rows, ("risk",)),
        impact_fields=_fields_matching(rows, ("impact",)),
