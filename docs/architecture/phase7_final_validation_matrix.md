...
| 7CM Index / Source Selection Runtime Workflow | Index/source-selection controls submit governed, validated payloads through a Phase 7 contract/service bridge. Local folder/file use OS picker metadata, Existing Run uses service-side DB lookup options, Object Storage uses service-side metadata validation, and active source configuration is user-facing rather than a raw state dump. | `docs/architecture/phase7_dashboard_runtime_interaction_wiring.md`, `src/learning/dashboard_runtime_interaction.py`, `scripts/dashboard_workflow_service.py`, `scripts/run_phase7_dashboard_runtime_interaction_validation.py`, and 7CM tests. | Satisfied for index/source-selection only when the 7CM validator passes. Broader dashboard blockers remain pending for 7CN–7CT. | Re-run 7CM validator after `scripts/run_analysis.py` regeneration; continue with 7CN–7CT before returning to 7CZ. |
