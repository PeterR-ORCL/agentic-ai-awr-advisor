...
class DashboardScreen6CandidateReviewPanelTests(unittest.TestCase):
    def test_html_dashboard_compiles(self) -> None:
        py_compile.compile(str(HTML_DASHBOARD_PATH), doraise=True)

    def test_screen6_candidate_review_preview_exists(self) -> None:
        source = read_text(HTML_DASHBOARD_PATH)
        rendered = self.render_screen6()
        for phrase in (
            "Screen 6 Learning Candidate Review Preview",
            "Read-Only Candidate Review Request Preview",
            "Phase 7BL Preview",
            "candidate_id",
            "candidate_type",
            "validation_status",
    ...
    def test_controls_are_disabled_preview_only(self) -> None:
        rendered = self.render_screen6()
        self.assertIn('id="screen6-candidate-review-preview-panel"', rendered)
        self.assertEqual(rendered.count("screen6-candidate-review-control preview-only"), 7)
        self.assertGreaterEqual(rendered.count("disabled"), 7)
        self.assertGreaterEqual(rendered.count('aria-disabled="true"'), 7)
        self.assertGreaterEqual(rendered.count('data-preview-only="true"'), 8)
        self.assertIn("Controls are disabled/preview-only", rendered)
        self.assertIn("Candidate review disabled in this phase", rendered)
    ...
    def test_no_fetch_xhr_forms_or_backend_calls(self) -> None:
        ...
        panel_source = inspect.getsource(
            dashboard._render_screen6_candidate_review_preview_panel
        ).lower()
