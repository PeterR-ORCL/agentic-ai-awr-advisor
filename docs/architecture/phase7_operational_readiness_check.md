...
## Known Dashboard Runtime Interaction Blocker

The checker also records this hard final certification blocker:

`DASHBOARD_RUNTIME_INTERACTION_NOT_WIRED`

Attempted 7CZ final certification found that the dashboard generated by `scripts/run_analysis.py` exposed visible controls that were inert, preview-only, or browser-local only. Those controls did not submit governed, validated payloads to a Phase 7 bridge/service path.

7CM is narrowed to index/source-selection runtime workflow only. It remediates the index portion of this blocker only when `scripts/run_phase7_dashboard_runtime_interaction_validation.py --json` passes. The validator requires generated/source index controls, the `src.learning.dashboard_runtime_interaction` contract, the local governed workflow service bridge, OS file/folder picker source metadata, service-side existing-run lookup, service-side Object Storage validation, dynamic active source configuration, and a clear submit/result path. Readiness must keep the broader dashboard runtime and selection workflow blockers pending for 7CN–7CT until Screens 1–6 and cross-screen integration are certified. It must also keep blockers active if dashboard controls only use disabled/preview/local-only behavior or if controls couple directly to `run_analysis.py`.
...
- Dashboard runtime interaction controls are wired to governed Phase 7 action paths and `DASHBOARD_RUNTIME_INTERACTION_NOT_WIRED` is absent.
